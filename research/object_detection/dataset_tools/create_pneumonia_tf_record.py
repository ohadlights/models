...
def create_example(raw_sample, images_root_dir, image_format):
    ...
    dcm_file = os.path.join(images_root_dir, image_id) + '.dcm'
    ...
    for box_orig in boxes:
        box_orig = [float(a) for a in box_orig]
        box = [box_orig[0], box_orig[1], box_orig[0] + box_orig[2], box_orig[1] + box_orig[3]]
        xmins += [float(box[0]) / float(width)]
        ymins += [float(box[1]) / float(height)]
        xmaxs += [float(box[2]) / float(width)]
        ymaxs += [float(box[3]) / float(height)]

        classes_text += [b'pneumonia']
        classes += [1]
...
def process_list(list_path, output_dir, images_root_dir, image_format, num_shards=1):
